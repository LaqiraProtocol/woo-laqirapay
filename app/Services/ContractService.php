...
class ContractService
{
    ...
    private function loadContractAbi(): array
    {
        ...
        if (!is_array($decoded)) {
            ...
            error_log('[LaqiraPay] Unable to load contract ABI: ' . ($context['json_error'] ?? 'unknown'));
    ...
    public function getCid(): ?string
    {
        ...
        try {
            ...
            if ($cid !== '') {
                ...
                LaqiraLogger::log(200, 'web3', 'cid_fetched', ['cid' => $cid]);
                return $cid;
            }
        } catch (Exception $e) {
            LaqiraLogger::log(400, 'web3', 'cid_fetch_failed', [], $e->getMessage());
    ...
    public function fetchCidFromContract(Contract $contract): string
    {
        $cid = '';
        $error = null;
        $contract->at(CONTRACT_ADDRESS)->call('getCid', function ($err, $data) use (&$cid, &$error) {
            if ($err !== null) {
                $error = $err;
                return;
            }
        ...
        try {
            ...
            if (!isset($this->abiCache[$abiPath])) {
                ...
                if (!is_array($decodedAbi)) {
                    ...
                    error_log('[LaqiraPay] Failed to decode network ABI: ' . ($context['json_error'] ?? 'unknown'));
                    return false;
                }
            ...
            $result = false;
            $contract->at($contractAddress)->call('isProviderNetworkActive', $address, function ($err, $data) use (&$result) {
            ...
            LaqiraLogger::log(400, 'web3', 'network_check_error', ['network' => $network['laqiraPayContract'] ?? 'unknown'], $e->getMessage());
            error_log('Error: ' . $e->getMessage());
            return false;
        }
    }
