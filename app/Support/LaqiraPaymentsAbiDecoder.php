 ...
 */
class LaqiraPaymentsAbiDecoder {
	...
	public static function parse_type( $type ) {
		...
		if ( is_array( $type ) ) {
			...
			if ( strpos( $type, 'bytes' ) === 0 && $type !== 'bytes' && ( $ret->size < 1 || $ret->size > 32 ) ) {
				throw new \Exception( sprintf( 'Invalid bytes<N> width: %d', (int) $ret->size ) );
			}
			if ( ( strpos( $type, 'uint' ) === 0 || strpos( $type, 'int' ) === 0 ) &&
				( $ret->size % 8 || $ret->size < 8 || $ret->size > 256 )
			) {
				throw new \Exception( sprintf( 'Invalid int/uint<N> width: %d', (int) $ret->size ) );
			}
			return $ret;
		}
	}
	...
	public static function decode_single( $parsed_type, $data, $data_bin, $offset ) {
		...
		if ( $parsed_type->name === 'address' ) {
			...
			return bin2hex( substr( $data_bin, $offset, ( $offset + $parsed_type->size ) - $offset ) );
				...
				$decoded_byte_length = (int) ( strlen( $num ) / 2 );
					if ( $decoded_byte_length > (int) $parsed_type->size ) {
						throw new \Exception(
							esc_html(
								sprintf(
									'Decoded int exceeds width: %d vs %d',
									(int) $parsed_type->size,
									$decoded_byte_length
								)
							)
						);
		...
		$type_name = is_string( $parsed_type->name ) ? $parsed_type->name : '';
		if ( function_exists( 'sanitize_key' ) ) {
			$type_name = sanitize_key( $type_name );
		} else {
			$type_name = preg_replace( '/[^a-z0-9_\[\]]/i', '', $type_name );
		}
			throw new \Exception( esc_html( sprintf( 'Unsupported or invalid type: %s', $type_name ) ) );
	}
}
