...
(function ($) {
    $(function () {
        ...
        var CART_REFRESH_ACTION = 'laqira_payments_update_cart_data';
        ...
        function ensureAppPositioned() {
            ...
            var placeholder = $('#startLaqiraPaymentsApp');
            if (!placeholder.length) {
                return;
            }

            if (!app.prev('#startLaqiraPaymentsApp').length) {
                app.insertAfter(placeholder);
            }
        }

        // Inject CSS to hide place order button when LaqiraPayments is active
        ...
        // Normalize slug and check if it's LaqiraPayments
        function isLaqiraMethod(slug) {
            if (!slug) return false;
            var s = String(slug).trim().toLowerCase().replace(/[^a-z0-9_]/g, '_');
            return s === 'laqira_payments' || s === 'laqira_payments_gateway' || s === 'wc_laqira_payments';
        }

        function getClassicPaymentMethod() {
            var checked = $('input[name="payment_method"]:checked').val() || null;

            if (checked) {
                return checked;
            }

            if (isOrderPayPage && $('#LaqirapayApp').length) {
                return 'laqira_payments';
            }

            return null;
        }

        function isLaqiraClassicSelected() {
            var selected = false;

            $('input[name="payment_method"]:checked').each(function () {
                if (isLaqiraMethod($(this).val())) {
                    selected = true;
                    return false;
                }
            });

            return selected || (isOrderPayPage && $('#LaqirapayApp').length);
        }
        ...
        function applyPaymentState(method, source) {
            if (!method) {
                method = getClassicPaymentMethod();
            }
            ...
            if (laqira) {
                ...
                // Refresh cart when LaqiraPayments is active
                if (
                    ...
                    if (!isOrderPayPage && isLaqiraClassicSelected() && $('input[name="payment_method"]:checked').closest('#order_review').length) {
                        get_refreshed_cart();
                    }
                }
            } else {
        ...
        $(document).ajaxComplete(function (event, xhr, settings) {
            ...
            if (!isRefreshRequest && requestUrl.includes('update_order_review') && !isOrderPayPage && isLaqiraClassicSelected()) {
                get_refreshed_cart();
            }
            $('#laqira_loder').remove();
            ensureAppPositioned();
            ...
            if (!isRefreshRequest) {
                var ajaxMethod = getClassicPaymentMethod();
        ...
        function initClassicMode() {
            var initial = getClassicPaymentMethod();
            ...
            $(document.body).on('change', 'input[name="payment_method"]', function () {
                ...
                if (!isOrderPayPage && isLaqiraClassicSelected() && $(this).closest('#order_review').length) {
                    get_refreshed_cart();
                }
            });
        ...
        var pollInterval = setInterval(function () {
            ...
            var method = detectBlocksMode() ? getActiveMethodFromBlocks() : getClassicPaymentMethod();
        ...
        setTimeout(function () {
            if (!isInitialized) {
                var timeoutMethod = detectBlocksMode() ? getActiveMethodFromBlocks() : getClassicPaymentMethod();
