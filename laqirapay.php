...
function laqirapay_action_links( array $actions ): array {
	$custom_actions = array(
		'settings' => '<a href="' . esc_url( wp_nonce_url( add_query_arg( array( 'page' => LAQIRAPAY_SETTINGS_PAGE ), admin_url( 'admin.php' ) ), 'laqirapay_settings_access' ) ) . '">' . esc_html__( 'Settings', 'laqirapay' ) . '</a>',
...
function check_laqirapay_requirements(): bool {
	...
	if ( version_compare( PHP_VERSION, '8.1', '<' ) ) {
		$errors[] = esc_html__( 'LaqiraPay requires PHP version 8.1 or higher.', 'laqirapay' );
	}
